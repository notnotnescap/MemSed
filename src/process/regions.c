...
ProcessRegions *process_regions_init(ProcessPid pid) {
  ...
  ProcessRegions *regions =
      malloc(sizeof(ProcessRegions) + sizeof(ProcessRegion) * capacity);
  ...
  while (fgets(line, sizeof(line), maps) != NULL) {
    ...
    if (sscanf(line, "%llx-%llx %c%c%c%c %x %*x:%*x %*u %n", &start, &end, &r,
               &w, &x, &s, &file_offset, &file_path_start) == 7) {
      char *file_path = line + file_path_start;

      if (count == capacity) {
        capacity *= 2;
        regions = realloc(regions, sizeof(ProcessRegions) +
                                       sizeof(ProcessRegion) * capacity);
      }
  ...
  fclose(maps);
  regions =
      realloc(regions, sizeof(ProcessRegions) + sizeof(ProcessRegion) * count);
  regions->regions_count = count;
