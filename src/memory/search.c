...
void memory_search_set_params(MemorySearch *memory_search,
                              MemorySearchParams params) {
...
static bool memory_search_should_process_type(MemoryType param, flt128_t value,
                                              MemoryType type) {
  if (type >= MemoryTypeMAX) {
    return false;
  }
  if (type == MemoryTypeUnsigned || type == MemoryTypeSigned ||
      type == MemoryTypeInteger || type == MemoryTypeFloating ||
      type == MemoryTypeNumber) {
    return false;
  }
...
static void memory_search_consolidate_results(void *context) {
  ...
  for (size_t set_i = 0; set_i < batch->sets_count; set_i++) {
    ...
    if (set->results_count > 0) {
      set->results =
          realloc(set->results, memory_search_get_result_size(set->type) *
                                    set->results_count);
    }
  }
...
static void memory_search_extend_results(MemorySearchResultSet *set,
                                         size_t *capacity) {
  if (set->results_count == *capacity) {
    *capacity *= 2;
    set->results = realloc(
        set->results, memory_search_get_result_size(set->type) * *capacity);
  }
}
...
static void *memory_search_begin_callback(void *context) {
  ...
  for (MemoryType type = MemoryTypeMAX - 1; type < MemoryTypeMAX; type--) {
    if (!memory_search_should_process_type(memory_search->params.type, value,
                                           type)) {
      continue;
    }
    ...
    set->results = malloc(memory_search_get_result_size(type) *
                          capacities[batch->sets_count]);
    max_type_size = MAX(max_type_size, memory_type_get_size(type));
    batch->sets_count++;
  }
  batch->sets =
      realloc(batch->sets, sizeof(MemorySearchResultSet) * batch->sets_count);

  memory_search->results.batches = batch;
  memory_search->results.batches_count++;
  memory_search->results.regions = regions;
  thread_self_push_cancel_cleanup(memory_search_consolidate_results,
                                  memory_search);
    ...
    while (addr < region->end) {
      if (addr > chunk_end_max_type_margin) {
        ...
        memory_search->search_progress =
            (flt32_t)(regions_progress + (addr - region->start)) /
            regions->total_size;
        chunk_addr = addr;
        size_t chunk_len =
            process_handle_read(handle, chunk_addr, chunk_buf, chunk_size);
        if (chunk_len == 0) {
          break;
        }
      ...
      for (size_t set_i = 0; set_i < batch->sets_count; set_i++) {
        ...
        case MemoryTypeU64:
          ...
          break;
        case MemoryTypeI8:
          if (*(int8_t *)chunk_cur < value_i8_min ||
              *(int8_t *)chunk_cur > value_i8_max) {
            continue;
          }
          if (chunk_end - addr < 1) {
            continue;
        ...
        case MemoryTypeI64:
          ...
          break;
        case MemoryTypeF32:
          if (isnan(*(flt32_t *)chunk_cur) ||
              *(flt32_t *)chunk_cur < (value_f32_min) ||
          ...
          break;
        case MemoryTypeF64:
          if (isnan(*(flt64_t *)chunk_cur) ||
              *(flt64_t *)chunk_cur < (value_f64_min) ||
          ...
          break;
        case MemoryTypeF128:
          if (isnan(*(flt128_t *)chunk_cur) ||
              *(flt128_t *)chunk_cur < (value_f128_min) ||
  ...
  thread_self_pop_cancel_cleanup(
      true); // memory_search_consolidate_results(memory_search)
  return NULL;
}
...
static void *memory_search_next_callback(void *context) {
  ...
  memory_search->results.batches =
      realloc(memory_search->results.batches,
              sizeof(MemorySearchResultBatch) *
  ...
  for (size_t last_set_i = 0; last_set_i < last_batch->sets_count;
       last_set_i++) {
  ...
  for (size_t last_set_i = 0; last_set_i < last_batch->sets_count;
    ...
    for (size_t result_i = 0; result_i < last_set->results_count; result_i++) {
      ...
      thread_self_quit_if_canceled();
      memory_search->search_progress = (flt32_t)(results_progress + result_i) /
                                       last_batch->total_results_count;
      ...
      case MemoryTypeU8:
        ...
        if (*(uint8_t *)value_buf < value_u8_min ||
            *(uint8_t *)value_buf > value_u8_max) {
          continue;
        }
        memory_search_extend_results(set, &capacities[set_i]);
        set->results_8[set->results_count].base.address = addr;
      ...
      case MemoryTypeI8:
        ...
        if (*(int8_t *)value_buf < value_i8_min ||
            *(int8_t *)value_buf > value_i8_max) {
          continue;
        }
        memory_search_extend_results(set, &capacities[set_i]);
        set->results_8[set->results_count].base.address = addr;
      ...
      case MemoryTypeI16:
        ...
        if (*(int16_t *)value_buf < value_i16_min ||
            *(int16_t *)value_buf > value_i16_max) {
          continue;
        }
        memory_search_extend_results(set, &capacities[set_i]);
        set->results_16[set->results_count].base.address = addr;
      ...
      case MemoryTypeI32:
        ...
        if (*(int32_t *)value_buf < value_i32_min ||
            *(int32_t *)value_buf > value_i32_max) {
          continue;
        }
        memory_search_extend_results(set, &capacities[set_i]);
        set->results_32[set->results_count].base.address = addr;
      ...
      case MemoryTypeI64:
        ...
        if (*(int64_t *)value_buf < value_i64_min ||
            *(int64_t *)value_buf > value_i64_max) {
          continue;
        }
        memory_search_extend_results(set, &capacities[set_i]);
        set->results_64[set->results_count].base.address = addr;
      ...
      case MemoryTypeF32:
        ...
        if (isnan(*(flt32_t *)value_buf) ||
            *(flt32_t *)value_buf < (value_f32_min) ||
      ...
      case MemoryTypeF64:
        ...
        if (isnan(*(flt64_t *)value_buf) ||
            *(flt64_t *)value_buf < (value_f64_min) ||
      ...
      case MemoryTypeF128:
        ...
        if (isnan(*(flt128_t *)value_buf) ||
            *(flt128_t *)value_buf < (value_f128_min) ||
...
static void *memory_search_update_callback(void *context) {
  ...
  size_t max_type_size = 0;
  if (memory_search->results.batches_count != 0) {
    batch = &memory_search->results
                 .batches[memory_search->results.batches_count - 1];
    for (size_t set_i = 0; set_i < batch->sets_count; set_i++) {
      MemorySearchResultSet *set = &batch->sets[set_i];
  ...
  while (true) {
    for (size_t item_i = 0; item_i < scratchpad->items_count; item_i++) {
      ...
      if (process_handle_read(handle, item->address, &item->value, size) !=
          size) {
    ...
    if (batch == NULL ||
        batch->total_results_count >= memory_search_update_max_results) {
      continue;
    }
    ...
    for (size_t set_i = 0; set_i < batch->sets_count; set_i++) {
      ...
      for (size_t result_i = 0; result_i < set->results_count; result_i++) {
        ...
        if (process_handle_read(handle, base->address, &base->value, size) !=
            size) {
...
void memory_search_begin(MemorySearch *memory_search) {
  ...
  memory_search->search_progress = 0.0f;
  memory_search->search_thread =
      thread_start(memory_search_begin_callback, memory_search);
}
...
void memory_search_next(MemorySearch *memory_search) {
  ...
  memory_search->search_progress = 0.0f;
  memory_search->search_thread =
      thread_start(memory_search_next_callback, memory_search);
}
...
memory_search_get_result_base(MemorySearchResultSet *set, size_t i) {
  switch (set->type) {
  case MemoryTypeUnsigned:
  case MemoryTypeSigned:
  case MemoryTypeInteger:
...
memory_search_get_display(MemoryAddress address, MemoryType type, void *value) {
  ...
  case MemoryTypeU8: {
    snprintf(display.value_str, sizeof(display.value_str), "%hhu",
             *(uint8_t *)value);
    break;
  }
  case MemoryTypeU16: {
    snprintf(display.value_str, sizeof(display.value_str), "%hu",
             *(uint16_t *)value);
    break;
  }
  case MemoryTypeU32: {
    snprintf(display.value_str, sizeof(display.value_str), "%u",
             *(uint32_t *)value);
    break;
  }
  case MemoryTypeU64: {
    snprintf(display.value_str, sizeof(display.value_str), "%llu",
             *(uint64_t *)value);
    break;
  }
  case MemoryTypeI8: {
    snprintf(display.value_str, sizeof(display.value_str), "%hhi",
             *(int8_t *)value);
    break;
  }
  case MemoryTypeI16: {
    snprintf(display.value_str, sizeof(display.value_str), "%hi",
             *(int16_t *)value);
    break;
  }
  case MemoryTypeI32: {
    snprintf(display.value_str, sizeof(display.value_str), "%i",
             *(int32_t *)value);
    break;
  }
  case MemoryTypeI64: {
    snprintf(display.value_str, sizeof(display.value_str), "%lli",
             *(int64_t *)value);
    break;
  }
  case MemoryTypeF32: {
    snprintf(display.value_str, sizeof(display.value_str), "%.*g", FLT_DIG,
             *(flt32_t *)value);
    break;
  }
  case MemoryTypeF64: {
    snprintf(display.value_str, sizeof(display.value_str), "%.*lg", DBL_DIG,
             *(flt64_t *)value);
    break;
  }
  case MemoryTypeF128: {
    snprintf(display.value_str, sizeof(display.value_str), "%.*Lg", LDBL_DIG,
             *(flt128_t *)value);
    break;
  }
  }

  snprintf(display.address_str, sizeof(display.address_str), "0x%" PRIXPTR,
           (uintptr_t)address);
...
memory_search_get_result_display(MemorySearchResultSet *set, size_t i) {
...
void memory_search_scratchpad_add(MemorySearch *memory_search,
                                  MemoryAddress addr, MemoryType type) {
  memory_search_stop_update(memory_search);
  if (!memory_search_process_is_attached(memory_search)) {
    return;
  }
  ...
  if (scratchpad->items_count > 0) {
    ...
    scratchpad->items =
        realloc(scratchpad->items, sizeof(MemorySearchScratchpadItem) *
                                       (scratchpad->items_count + 1));
  } else {
    scratchpad->items = malloc(sizeof(MemorySearchScratchpadItem) * 1);
  }

  MemorySearchScratchpadItem *item =
      &scratchpad->items[scratchpad->items_count];
  item->address = addr;
  item->type = type;
  item->active = false;
  ...
  if (process_handle_read(memory_search->handle, addr, &item->value, size) !=
      size) {
    memset(&item->value, 0, size);
  }
...
memory_search_get_scratchpad(MemorySearch *memory_search) {
  return &memory_search->scratchpad;
}

MemorySearchResultDisplay
memory_search_get_scratchpad_display(MemorySearchScratchpadItem *item) {
  return memory_search_get_display(item->address, item->type, &item->value);
}

void memory_search_scratchpad_set(MemorySearch *memory_search,
                                  MemorySearchScratchpadItem *item,
                                  flt128_t value) {
  memory_search_stop_update(memory_search);
  if (!memory_search_process_is_attached(memory_search)) {
  ...
  case MemoryTypeU8: {
    uint8_t val = value;
    process_handle_write(memory_search->handle, item->address, &val,
                         sizeof(val));
    break;
  }
  case MemoryTypeU16: {
  ...
  size_t size = memory_type_get_size(item->type);
  if (process_handle_read(memory_search->handle, item->address, &item->value,
                          size) != size) {
    memset(&item->value, 0, size);
  }
}

void memory_search_scratchpad_del(MemorySearch *memory_search,
                                  MemorySearchScratchpadItem *item) {
  memory_search_stop_update(memory_search);
  if (!memory_search_process_is_attached(memory_search)) {
    return;
  ...
  if (scratchpad->items_count == 0) {
    ...
    memmove(&scratchpad->items[item_i], &scratchpad->items[item_i + 1],
            sizeof(MemorySearchScratchpadItem) *
                (scratchpad->items_count - item_i));
    scratchpad->items =
        realloc(scratchpad->items,
...
void memory_search_tick(MemorySearch *memory_search) {
  if (memory_search_is_searching(memory_search)) {
    ...
    } else if (memory_search->update_thread == NULL &&
               ((memory_search->results.current_results_count > 0 &&
                 memory_search->results.current_results_count <
                     memory_search_update_max_results) ||
                memory_search->scratchpad.items_count > 0)) {
